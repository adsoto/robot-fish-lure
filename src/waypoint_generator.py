...
#     return np.array(new_wypt), start_time, start_pos, new_theta
...
def turn(radians, angular_vel, start_time, start_pos, current_theta):
    ...
    turn_t = radians/angular_vel
    num_pts = round(turn_t/dt)
    print(num_pts)
    new_start_time = start_time+turn_t
    full_rotations = int(radians/np.pi*2)
    final_theta = "calc final theta"
    wypts = np.array([])
    for i in range(1, num_pts+1):
        ...
        wypts = np.append(wypts, [new_time, start_pos[0], start_pos[1], new_theta])
